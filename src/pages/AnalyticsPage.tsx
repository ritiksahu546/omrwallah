...
export const AnalyticsPage: React.FC = () => {
  const hasData = MOCK_STUDENT_STATS.testsTaken > 0;
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs">
          ...
          <div className="text-2xl font-black text-slate-900">{hasData ? '2.4 sec' : '0.0 sec'}</div>
          <span className="text-[11px] font-bold text-slate-400">
            {hasData ? '▲ 35% faster than last month' : 'No tests evaluated yet'}
          </span>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs">
          ...
          <div className="text-2xl font-black text-slate-900">{hasData ? '78.4%' : '0%'}</div>
          <span className="text-[11px] font-bold text-slate-400">
            {hasData ? '▲ Top 12% in NEET Mock Cohort' : 'Awaiting first test'}
          </span>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs">
          ...
          <div className="text-2xl font-black text-slate-900">{hasData ? '-14 avg' : '0 avg'}</div>
          <span className="text-[11px] font-bold text-slate-400">
            {hasData ? '▼ Down from -28 in April' : '0 penalty marks'}
          </span>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs">
          ...
          <div className="text-2xl font-black text-slate-900">{hasData ? '1,240' : '0'}</div>
      ...
      {!hasData ? (
        <div className="bg-white border border-slate-200 rounded-3xl p-10 sm:p-14 text-center max-w-lg mx-auto space-y-4 shadow-xs">
          ...
          <h3 className="font-extrabold text-lg sm:text-xl text-slate-900">No Analytics Available Yet</h3>
          <p className="text-xs sm:text-sm text-slate-500 leading-relaxed">
            Take practice tests or scan your physical OMR answer sheets. Your mistake pattern diagnostic, accuracy progression, and subject readiness analysis will automatically appear here.
          </p>
        </div>
      ) : (
